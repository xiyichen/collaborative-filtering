...
class VAE_trainer:
		...
		with tqdm(total=len(dataloader) * num_epochs) as pbar:
			for epoch in range(num_epochs):
				for idx, (data_batch, mask_batch, user_id_batch) in enumerate(dataloader):
					optimizer.zero_grad()
					reconstructed_batch, _, mu, logvar = self.model(data_batch, user_id_batch)
					beta = self.model.get_beta(epoch, num_epochs)
					loss, recon_loss, kl_loss = self.model.loss_function(data_batch, reconstructed_batch, mask_batch, mu, logvar, beta)
				...
				if use_validation:
					...
					pbar.set_description('At epoch {:3d}: best loss {:.4f}, current loss {:.4f}, current MMSE loss {:.4f}, current KL loss {:.4f}, current LR {:.5f}, current beta {:.4f}'
						.format(epoch, best_rmse, reconstruction_rmse, recon_loss, kl_loss, optimizer.param_groups[0]["lr"], beta))
					self.model.train()
				scheduler.step()
	...
	def predict(self, data, user_id):
		data_reconstructed, _, _, _ = self.model(data, user_id)
